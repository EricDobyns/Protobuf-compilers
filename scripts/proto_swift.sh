...
echo "Compiling Protobuf packages for Swift..."
for file in $PROTO_PATH/*.proto; do
	...
	# Generate pb files
	protoc -I $PROTO_PATH $PROTO_PATH/$PACKAGE_NAME.proto --swift_out=$OUTPUT_DIR
	protoc -I $PROTO_PATH $PROTO_PATH/$PACKAGE_NAME.proto --grpc-swift_out=Client=true,Server=false:$OUTPUT_DIR
done
